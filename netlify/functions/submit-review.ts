...
interface SubmissionData {
  ...
  responses: ArtworkResponse[] | Record<string, ArtworkResponse>;
...
async function uploadToCloudinary(imageData: string, fileName: string): Promise<string> {
  ...
  // Create signature - must match exact parameter order
...
function normalizeResponses(responses: ArtworkResponse[] | Record<string, ArtworkResponse>): ArtworkResponse[] {
  if (Array.isArray(responses)) {
    return responses;
  }

  // If it's an object/record, convert to array
  if (responses && typeof responses === 'object') {
    return Object.entries(responses).map(([key, value]) => {
      // Handle case where value might just be a rating number
...
function generateCSV(data: SubmissionData, imageUrls: string[]): string {
  const lines: string[] = [];
  const responses = normalizeResponses(data.responses);
...
const handler: Handler = async (event) => {
  ...
  try {
    ...
    console.log('Received submission from:', data.reviewerName);
    console.log('Responses type:', typeof data.responses, Array.isArray(data.responses));
    ...
    if (!data.reviewerName) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Missing reviewer name' }),
      };
    }
    ...
    if (data.uploadedImages && data.uploadedImages.length > 0) {
      console.log('Uploading', data.uploadedImages.length, 'images');
    ...
    const emailHtml = `
      ...
      <h2>Summary</h2>
      <ul>
        <li>Total artworks reviewed: ${responses.length}</li>
    ...
    // Send email
    const recipientEmail = process.env.RECIPIENT_EMAIL || process.env.NOTIFICATION_EMAIL || 'alex@pearhaus.com';
    const fromEmail = process.env.SENDGRID_FROM_EMAIL || 'noreply@pearhaus.com';
